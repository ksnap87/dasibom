...
import {
  ...
  RefreshControl, Alert, SafeAreaView, Animated, PanResponder,
} from 'react-native';
import { useNavigation } from '@react-navigation/native';
import AppText from '../components/AppText';
import AsyncStorage from '@react-native-async-storage/async-storage';
import api, { expressInterest, dailyCheckin } from '../api/client';
...
import {
  Button, Tag, ScreenHeader,
  colors, radius, spacing, typography, shadow,
} from '../theme';
      ...
      onPanResponderRelease: (_, g) => {
        ...
        if (g.dx > SWIPE_THRESHOLD) {
          ...
          Animated.spring(translateX, {
            toValue: 0, useNativeDriver: true, friction: 7, tension: 60,
          }).start();
        }
      },
...
export default function SuggestionsScreen() {
  ...
  const [, setPreviewQuestions] = useState<string[]>(DEFAULT_PREVIEW_QUESTIONS);
  const [, setRequiredConditions] = useState<string[]>([]);
  const [dailyCount, setDailyCount] = useState(0);
  const [dailyLimit, setDailyLimit] = useState(DAILY_LIMIT);
  const [dailyDone, setDailyDone] = useState(false);
  const [, setMyProfile] = useState<any>(null);
  const [checkinStreak, setCheckinStreak] = useState(0);
  const [, setCheckinReward] = useState(0);
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.empty}>
          ...
          <View style={styles.emptyButtons}>
            <Button
              label={`💎 크레딧으로 ${EXTRA_PER_CREDIT}명 더 보기`}
              variant="secondary"
  ...
  const headerRight = (
    <Tag tone="accent" label={`오늘 ${dailyCount}/${dailyLimit}`} />
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScreenHeader
        title="추천 상대"
        subtitle={suggestions.length > 0 ? `${suggestions.length}명 남음` : undefined}
        right={headerRight}
      />
...
const styles = StyleSheet.create({
  ...
  qaCategory: { marginBottom: spacing.sm },
  qaCategoryTitle: {
    fontSize: typography.caption,
    fontWeight: typography.bold,
    color: colors.primaryDark,
